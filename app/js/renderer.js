...
async function renderOverview() {
    ...
    // Refresh dashboard widgets if available
    if (window.dashboardWidgets && window.dashboardWidgets.loadDashboardData) {
        await window.dashboardWidgets.loadDashboardData();
    }
}

function renderRecentTransactions(transactions) {
    const container = document.getElementById('recent-transactions');
    if (!container) return; // Element doesn't exist in current dashboard layout
