...
document.addEventListener('DOMContentLoaded', function () {
        ...
        setTimeout(() => {
                ...
                // THEN load data after charts are ready
                setTimeout(() => {
                        console.log('📊 Charts ready, now loading data...');
                        loadDashboardData();
                }, 100);
        }, 100);
});
...
async function loadDashboardData() {
        try {
                ...
                // Get data from storage using electronAPI
                const transactionsResult = await window.electronAPI.getTransactions(userId);
                const tasksResult = await window.electronAPI.getTasks(userId);
                const billsResult = await window.electronAPI.getBills(userId);
                const goalsResult = await window.electronAPI.getGoals(userId);
                const notesResult = await window.electronAPI.getNotes(userId);
                const eventsResult = await window.electronAPI.getEvents(userId);

                const transactions = transactionsResult.success ? transactionsResult.transactions : [];
                const tasks = tasksResult.success ? tasksResult.tasks : [];
                const bills = billsResult.success ? billsResult.bills : [];
    ...
    ${todayTasks.map(task => `
     <li class="item">
      <div class="item-content">
       <span class="priority-badge priority-${(task.priority || 'Normal').toLowerCase()}">${task.priority || 'Normal'}</span>
       <span class="item-title">${escapeHtml(task.taskTitle)}</span>
      </div>
  ...
  `;
   ...
   <ul class="item-list">
    ${upcomingBills.map(bill => `
     <li class="item">
      <div class="item-content">
       <span class="item-title">${escapeHtml(bill.billName)}</span>
      </div>
...
function updateGoalsWidget(goals) {
        const activeGoals = goals.filter(g => !g.isAchieved).slice(0, 3);
  ...
  `;
    ...
    <div class="goal-item">
     <div class="goal-header">
      <span class="goal-title">${escapeHtml(goal.goalTitle)}</span>
  ...
  `;
   ...
   <ul class="item-list">
    ${upcomingEvents.map(event => `
     <li class="item">
      <div class="item-content">
       <span class="item-title">${escapeHtml(event.eventTitle)}</span>
      </div>
      <span class="item-time">${formatDate(event.eventDate)}</span>
     </li>
    `).join('')}
   </ul>
  `;
...
function updateChartData(transactions) {
        updateExpenseChartData(transactions);
        updateTrendChartData(transactions);
}

// Update expense breakdown chart with real data
function updateExpenseChartData(transactions) {
        console.log('📊 updateExpenseChartData called with', transactions.length, 'transactions');

        if (!expenseChartInstance) {
                console.warn('⚠️ Expense chart instance not found');
                return;
        }
        ...
        console.log('💸 Monthly expenses filtered:', monthlyExpenses.length);
...
// Update income vs expense trend chart with real data
function updateTrendChartData(transactions) {
        ...
        // Update chart
        trendChartInstance.data.labels = monthlyData.map(m => m.label);
        trendChartInstance.data.datasets[0].data = monthlyData.map(m => m.income);
        trendChartInstance.data.datasets[1].data = monthlyData.map(m => m.expenses);
